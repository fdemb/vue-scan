...
/** Component instances, keyed by uid - used to get current props/state */
...
/** Currently selected component UID for targeted state tracking.
 *  When null, no state snapshots are being recorded. */
export const selectedComponentUid = signal<number | null>(null);

/** Revision counter for tree structure changes (mount/unmount) */
export const treeRevision = signal(0);
...
function snapshotRawState(
  ...
  for (const key of Object.keys(rawSetupState)) {
    ...
    if (raw !== null && typeof raw === "object" && !isRef(raw) && !(raw as any).__v_isReactive) {
      if ("render" in (raw as any) || "setup" in (raw as any) || "__asyncLoader" in (raw as any))
        continue;
    }
...
/** Register a new component in the tree.
...
export function trackComponentAdd(
  ...
  // Store instance data for later retrieval
  instanceMap.set(uid, { props, rawSetupState });

  // Only capture initial state snapshot if this is the selected component
  if (selectedComponentUid.value === uid) {
...
export function trackComponentRemove(uid: number) {
  ...
  // If removed component was selected, clear selection
  if (selectedComponentUid.value === uid) {
    selectedComponentUid.value = null;
  }
 ...
 *  then update the snapshot. Only tracks if this is the selected component.
 *  Call on component:updated. */
export function trackComponentUpdate(
  ...
  // Update component info in map (name/parentUid may change)
  ...
  instanceMap.set(uid, { props: currentProps, rawSetupState });

  // Only track detailed state changes for the selected component
  if (selectedComponentUid.value !== uid) {
    return;
  }
...
/** Select a component for detailed state tracking.
 *  Clears any previous tracking data and starts fresh.
 *  Uses the stored instance data from the most recent update. */
export function selectComponent(uid: number) {
  // Clear previous selection's data
  if (selectedComponentUid.value !== null) {
    const prevNode = componentMap.get(selectedComponentUid.value);
    if (prevNode) {
      prevNode.updates = [];
      prevNode.totalUpdates = 0;
    }
    lastSnapshots.delete(selectedComponentUid.value);
  }

  selectedComponentUid.value = uid;

  // Clear new selection's data and capture initial snapshot
  const node = componentMap.get(uid);
  if (node) {
    node.updates = [];
    node.totalUpdates = 0;
  }

  // Get the stored instance data
  const instanceData = instanceMap.get(uid);
  if (instanceData) {
    lastSnapshots.set(uid, {
      props: shallowClone(instanceData.props),
      state: snapshotRawState(instanceData.rawSetupState),
    });
  }
...
/** Get the selected component node, if any. */
export function getSelectedComponent(): ComponentNode | null {
  if (selectedComponentUid.value === null) return null;
  return componentMap.get(selectedComponentUid.value) ?? null;
}

/** Build a tree structure from all tracked components. */
export function getComponentTree(): ComponentTreeNode[] {
  ...
  const nodeMap = new Map<number, ComponentTreeNode>();
  ...
  for (const node of nodes) {
    ...
    if (node.parentUid === undefined || !nodeMap.has(node.parentUid)) {
      // This is a root node
      roots.push(treeNode);
    } else {
  ...
  function sortChildren(node: ComponentTreeNode) {
    node.children.sort((a, b) => a.uid - b.uid);
    for (const child of node.children) {
      sortChildren(child);
    }
  }

  roots.sort((a, b) => a.uid - b.uid);
