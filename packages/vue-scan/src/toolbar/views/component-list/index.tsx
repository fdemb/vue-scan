  ...
  return (
    <div class="flex h-[360px]">
      ...
      <div class="flex-1 overflow-y-auto min-w-0">
        {selectedNode ? <DetailView node={selectedNode} /> : <EmptyState />}
      </div>
    </div>
  );
}
...
function DetailView({
  ...
  node: { uid: number; name: string; updates: ComponentUpdate[]; totalUpdates: number };
}) {
  return (
    <div class="flex flex-col h-full">
      ...
      <div class="px-3 py-2 border-b border-white/[0.06]">
        ...
        <div class="text-[10px] text-neutral-500 mt-1">
          {node.totalUpdates === 0 ? (
            ...
            <span>
              {node.totalUpdates} update{node.totalUpdates !== 1 ? "s" : ""} since selection
            </span>
          )}
        </div>
      </div>
      ...
      <div class="flex-1 overflow-y-auto">
        {node.updates.length === 0 ? (
          ...
          [...node.updates]
            .reverse()
            .map((update: ComponentUpdate, i: number) => (
    ...
    <div class="px-3 py-2 border-b border-white/[0.04]">
      ...
      {!hasChanges ? (
        ...
        <div class="flex flex-col gap-1.5">
          {props.length > 0 && <ChangeSection label="Props" changes={props} color="blue" />}
          {state.length > 0 && <ChangeSection label="State" changes={state} color="purple" />}
        </div>
      )}
    </div>
  );
